...
PanelWindow {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: function(mouse) {
                                        if (musicPanel.length > 0 && !seekProc.running) {
                                            ...
                                            seekProc.command = ["playerctl", "--player=spotify", "position", seekPos.toString()]
    ...
    Process {
        id: musicLenProc
        command: ["sh", "-c", "playerctl --player=spotify metadata mpris:length 2>/dev/null | awk '{print $1/1000000}'"]
    ...
    Process {
        id: seekProc
        command: ["playerctl", "--player=spotify", "position", "0"]
    }
}
