...
PanelWindow {
    ...
    Process {
        command: [
            "bash",
            "-c",
            "while true; do inotifywait -e close_write /tmp/bar-hidden >/dev/null; cat /tmp/bar-hidden; done"
        ]
    ...
    Process {
        id: mediaProc
        command: ["bash", "-c", "status=$(playerctl --player=spotify status 2>/dev/null); pos=$(playerctl --player=spotify position 2>/dev/null | cut -d. -f1); len=$(playerctl --player=spotify metadata mpris:length 2>/dev/null); len=$((len / 1000000)); if [ \"$status\" = \"Playing\" ] || [ \"$status\" = \"Paused\" ]; then artist=$(playerctl --player=spotify metadata artist 2>/dev/null); title=$(playerctl --player=spotify metadata title 2>/dev/null); if [ -n \"$title\" ]; then text=\"$title\"; [ -n \"$artist\" ] && text=\"$artist - $title\"; if [ ${#text} -gt 35 ]; then text=\"${text:0:32}...\"; fi; echo \"$status|$text|$pos|$len\"; else echo 'stopped||0|0'; fi; else echo 'stopped||0|0'; fi"]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split("|")
                if (parts.length >= 4) {
    ...
    Process {
        id: networkProc
        command: ["bash", "-c", "wifi=$(nmcli -t -f active,ssid,signal dev wifi 2>/dev/null | grep '^はい' | head -1); if [ -n \"$wifi\" ]; then ssid=$(echo \"$wifi\" | cut -d: -f2); sig=$(echo \"$wifi\" | cut -d: -f3); echo \"1|$ssid|$sig\"; else echo '0||0'; fi; bt='0'; devices=$(echo -e 'devices\\nquit' | bluetoothctl 2>/dev/null | grep '^Device' | awk '{print $2}'); for mac in $devices; do if echo -e \"info $mac\\nquit\" | bluetoothctl 2>/dev/null | grep -q 'Connected: yes'; then bt='1'; break; fi; done; echo \"bt:$bt\""]
        stdout: SplitParser {
            onRead: data => {
                var line = data.trim()
    ...
    Process {
        ...
        # Disk (root partition free space in GB)
        ...
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split("|")
                if (parts.length >= 3) {
                    bar.cpuPercent = parseFloat(parts[0]) || 0
                    bar.ramPercent = parseFloat(parts[1]) || 0
                    bar.diskFree = parts[2] + "G"
                }
            }
        }
    }
    ...
    Item {
            ...
            Notch {
                ...
                Timer {
                    ...
                    onTriggered: {
                        ...
                        clockLabel.text = ampm + hour12 + ":" + (minutes < 10 ? "0" + minutes : minutes)
                    }
                }
            }
                            ...
                            Repeater {
                                ...
                                delegate: Item {
                                    ...
                                    Text {
                                        ...
                                        anchors.centerIn: parent
                                        text: {
                                            var num = modelData.id
                                            var kanji = ["零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"]
                                            if (num <= 10) return kanji[num]
                                            else return modelData.name || modelData.id.toString()
                                        }
        ...
        Row {
            ...
            // Add this component before the battery notch in rightSection
            Notch {
                width: sysRow.width + 24
                hovered: sysMA.containsMouse
