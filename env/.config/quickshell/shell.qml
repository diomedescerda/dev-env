...
ShellRoot {
    ...
    property string wallpaperPath: homePath + "/misc/wallpapers"
    ...
    Process {
        id: wifiCurrentProc
        command: ["bash", "-c", "nmcli -t -f active,ssid,signal dev wifi 2>/dev/null | grep '^yes' | head -1 | sed 's/^yes://'"]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split(":")
                if (parts.length >= 2) {
                    root.wifiCurrentSSID = parts[0]
    ...
    Process {
        id: wifiScanProc
        command: ["bash", "-c", "nmcli -t -f ssid,signal,security dev wifi list --rescan yes 2>/dev/null | head -30"]
        stdout: SplitParser {
            onRead: data => {
                var line = data.trim()
                if (line.length === 0) return
                ...
                // Add if it doesn't exist (includes current network)
                if (!exists) {
                    current.push({ 
                        ssid: ssid, 
                        signal: signal, 
                        security: security,
                        isConnected: ssid === root.wifiCurrentSSID // Mark if connected
                    })
